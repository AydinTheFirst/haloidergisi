...
export default function PostCard({ className, post }: PostCardProps) {
  const likes =
    post.reactions?.filter((reaction) => reaction.type === ReactionType.LIKE).length || 0;
      ...
      <CardBody className="p-0">
        <div className="flex gap-3">
          <div className="col-span-12 lg:col-span-4">
            {post.cover && (
              <CdnImage className="h-full w-40 object-cover" radius="none" src={post.cover} />
            )}
          </div>
          <div className="flex-1">
            <div className="flex h-full flex-col gap-3 p-2">
              <div className="flex flex-1 flex-col gap-2">
                ...
                <p className="text-muted line-clamp-1 xl:line-clamp-3">{post.description}</p>
              </div>
