...
import { User, Profile, Crew } from "@repo/db";
...
function RouteComponent() {
  ...
  const { data: users } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await apiClient.get<QueryRes<_User>>("/users", {
        params: {
          fields: JSON.stringify({ profile: true, crew: true }),
          page,
          limit,
          search: q,
        },
  ...
  const columns: ColumnDef<_User>[] = [
    ...
    },
    {
      ...
      header: "Crew",
      cell: ({ row }) => row.original.crew?.name || "-",
    },
    {
  ...
  return (
    <section className='space-y-4'>
      <div className='flex items-center justify-between'>
        <Button render={<Link to='/dashboard/users/new' />}>Yeni</Button>
